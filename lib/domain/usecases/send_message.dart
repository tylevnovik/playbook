...
class SendMessage {
  ...
  Future<Either<Failure, Message>> call({
    ...
    return await configResult.fold((failure) async => Left(failure), (
      config,
    ) async {
      // 2. Load recent messages
      final messagesResult = await chatRepository.getMessages(chatId);
      return await messagesResult.fold((failure) async => Left(failure), (
        messages,
      ) async {
        // 3. Save user message
        ...
        return await saveUserResult.fold((failure) async => Left(failure), (
          _,
        ) async {
          // 4. Send to LLM
          ...
          return await responseResult.fold((failure) async => Left(failure), (
            responseContent,
          ) async {
